...
function Chessman(id, type, row, col, faction) {
  ...
  this.move = function (dcol, drow) {

    function moveIsValid(mrow, drow, mcol, drow, options) {
      let valid = false;

      if ((mrow == drow) && (mcol == dcol)) {
        // if logical position is the same, no move was made
        valid = false;
      }
      options.forEach(option => {
        if (option.row == drow && option.col == dcol) {
          //todo check faction
          valid = true;
        }
      })
    ...
    if (moveIsValid(this.position.logical.row, drow, this.position.logical.col, dcol, this.options)) {
      //If valid, Check if enemy chessman has been killed

      //check if desination contains an enemy
      let target = chessmen.find(chessman => {
        return (chessman.position.logical.row == drow && chessman.position.logical.col == dcol)
      })
      ...
      this.derender();
      this.position.logical.col = dcol;
      this.position.logical.row = drow;
