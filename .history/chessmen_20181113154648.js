...
function Chessman(id, type, row, col, faction) {
  ...
  this.derender = function (ctx) {
    ctx.clearRect(this.position.graphical.x, this.position.graphical.y, TILE_SIZE * 3, TILE_SIZE * 3);
  }
...
function myDown(e) {
  ...
  for (i = 0; i < chessmen.length; i++) {
    if (coordsMatch(pos, chessmen[i].position.graphical)) {
      ...
      selector.style.top = e.clientY - TILE_SIZE * 3 / 2 + "px";
      selector.style.left = e.clientX - TILE_SIZE * 3 / 2 + "px";
...
function myUp(e) {
  ...
  selector_ctx.clearRect(0, 0, TILE_SIZE * 3, TILE_SIZE * 3);
...
function myMove(e) {
  if (dragOk) {
    ...
    e.preventDefault();
    e.stopPropagation();
    let x = parseInt(selector.style.top.replace(/\D/g, '')) + TILE_SIZE * 3 / 2; //remove 'px'
    let y = parseInt(selector.style.left.replace(/\D/g, '')) + TILE_SIZE * 3 / 2; //remove 'px'


    //todo just move selected chess piece canvas
    let top = parseInt(selector.style.top.replace(/\D/g, '')) + TILE_SIZE * 3 / 2; //remove 'px'
    let left = parseInt(selector.style.left.replace(/\D/g, '')) + TILE_SIZE * 3 / 2; //remove 'px'
    ...
    //update css
    selector.style.top = top - TILE_SIZE * 3 / 2 + 'px';
    selector.style.left = left - TILE_SIZE * 3 / 2 + 'px';
