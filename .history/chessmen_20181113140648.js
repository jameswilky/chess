...
function myDown(e) {
  ...
  for (i = 0; i < chessmen.length; i++) {
    if (coordsMatch(pos, chessmen[i].position.logical)) {
      ...
      selector.style.top = (chessmen[i].position.graphical.y - shiftY + "px");
      selector.style.left = (chessmen[i].position.graphical.x - shiftX + "px");
...
function myMove(e) {
  if (dragOk) {
    ...
    let top = parseInt(selector.style.top.replace(/\D/g, '')); //remove 'px'
    let left = parseInt(selector.style.left.replace(/\D/g, '')); //remove 'px'
