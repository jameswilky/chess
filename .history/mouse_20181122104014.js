...
let k = Math.floor((window.innerWidth - WIDTH) / 2)

//Listen for when we hover over a chest piece, then draw the movement options
chessmen_layer.addEventListener('mousemove', (e) => {
  if (dragging == false) {
    let pos = {
      x: e.clientX - Math.floor((window.innerWidth - WIDTH) / 2),
      y: e.clientY - Math.floor((window.innerHeight - HEIGHT) / 2)
    }
  ...
  chessmen.forEach(chessman => {
    if (coordsMatch(pos, chessman.position.graphical)) {
      ...
      function onMouseMove(moveEvent) {
        let x = moveEvent.clientX - Math.floor((window.innerWidth - WIDTH) / 2);
        let y = moveEvent.clientY - Math.floor((window.innerHeight - HEIGHT) / 2);
        moveAt(x, y);
      }
      ...
      chessmen_layer.onmouseup = function (upevent) {
        ...
        //get offset
        let x = upevent.clientX - Math.floor((window.innerWidth - WIDTH) / 2);
        let y = upevent.clientY - Math.floor((window.innerHeight - HEIGHT) / 2);
