  ...
  this.calculate_movement_options = function () {
    ...
    if (this.type == "pawn") {
      if (this.faction == "black") {
        check_tile(r + 1, c, false, true);//south
        check_tile(r + 1, c + 1, true, true) // south East
        check_tile(r + 1, c - 1, true, true) //south West
        if (this.nMoves == 0) {
          check_tile(r + 2, c, false, true); //Extra move on first turn
        }
      }
      if (this.faction == "white") {
        check_tile(r - 1, c, false, true); //north
        check_tile(r - 1, c + 1, true, true) // North East
        check_tile(r - 1, c - 1, true, true) //North West
        if (this.nMoves == 0) {
          check_tile(r - 2, c, false, true); //Extra move on first turn
        }
      }
