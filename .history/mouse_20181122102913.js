...
chessmen_layer.addEventListener('mousemove', (e) => {
  if (dragging == false) {
    let pos = {
      x: e.clientX - Math.floor((document.body.getElementsByClassName("main")[0].getElementsByClassName("main")[0].clientWidth - WIDTH) / 2),
      y: e.clientY - Math.floor((document.body.getElementsByClassName("main")[0].getElementsByClassName("main")[0].clientHeight - HEIGHT) / 2)
    }
...
chessmen_layer.addEventListener('mousedown', (e) => {
  ...
  chessmen.forEach(chessman => {
    if (coordsMatch(pos, chessman.position.graphical)) {
      ...
      document.body.getElementsByClassName("main")[0].getElementsByClassName("main")[0].append(selector);
      ...
      function onMouseMove(moveEvent) {
        let x = e.clientX - Math.floor((document.body.getElementsByClassName("main")[0].getElementsByClassName("main")[0].clientWidth - WIDTH) / 2);
        let y = e.clientY - Math.floor((document.body.getElementsByClassName("main")[0].getElementsByClassName("main")[0].clientHeight - HEIGHT) / 2);
