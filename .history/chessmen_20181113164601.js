...
function moveIsValid(r1, r2, c1, c2, chessman) {
  ...
  if ((chessman.position.logical.row == row) && (chessman.position.logical.col == col)) {
    // if logical position is the same, no move was made
    console.log("didnt move");
    valid = false;
  }
  chessman.options.forEach(option => {
    if (option.row == row && option.col == col) {
      //todo check faction
...
function moveChessman(col, row, chessman) {
  moveIsValid(chessman.position.logical.row, row, chessman.position.logical.col, col, chessman);
